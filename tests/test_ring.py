...
def run_checked(sim, duration):
    ...
    def check(s):
        nonlocal min_gap
        min_gap = min(min_gap, s.lane.min_gap())
...
def test_no_collisions_and_count_conserved(n):
    ...
    assert sorted(v.id for v in sim.vehicles) == list(range(n))


def test_positions_stay_on_ring():
    sim = ring_road(30, seed=2)
    sim.run(100.0)
    assert all(0.0 <= v.position < sim.road.length for v in sim.vehicles)
